				...
				if( ( Tested_resultCode == "AllTested" ) or (Tested_resultCode == TestData[i].resultCode) ) then
					if(mob_request.single == true)then
					    ...
					    if( (mob_request.name ~= "StartStream") and (mob_request.name ~= "StopStream") and 
							...
							--ToDo: shall be uncommened and debugged when APPLINK-16610 is fixed
							--Preconditions:
							if(mob_request.name == "StopStream" and Tested_resultCode ~= "SUCCESS") then
							...
							if(mob_request.name == "StopAudioStream" and Tested_resultCode ~= "SUCCESS") then
