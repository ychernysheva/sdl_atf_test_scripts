...
function Test:initHMI()
    ...
    EXPECT_HMIEVENT(events.connectedEvent, "Connected websocket"):Do(
        function()
...
function Test:initHMI_onReady()
    local function ExpectRequest(name, mandatory, params)
        ...
        return EXPECT_HMIEVENT(event, name):Times(mandatory and 1 or AnyNumber()):Do(
            function(_, data)
                xmlReporter.AddMessage(
                    "hmi_connection",
                    "SendResponse",
                    {
    ...
    local function ExpectNotification(name, mandatory)
        ...
        return EXPECT_HMIEVENT(event, name):Times(mandatory and 1 or AnyNumber())
    end
    ...
    self.applications = {}
    ExpectRequest("BasicCommunication.UpdateAppList", false, {}):Pin():Do(
        function(_, data)
...
function Test:connectMobile(device, exitOnDisconnect)
...
function Test:startSession(session)
    session:Start()
...
function Test.hmiConnection:EXPECT_HMIRESPONSE(id, args)
    ...
    ret:ValidIf(
        function(self, data)
            ...
            xmlReporter.AddMessage(
                "EXPECT_HMIRESPONSE",
                {["Id"] = tostring(id), ["Type"] = "EXPECTED_RESULT"},
                arguments
            )
        ...
        ret:ValidIf(
            function(self, data)
                ...
                xmlReporter.AddMessage(
                    ...
                    {["Id"] = correlation_id, ["name"] = tostring(name), ["Type"] = "EXPECTED_RESULT"},
                    arguments
                )
                xmlReporter.AddMessage(
                    "EXPECT_HMINOTIFICATION",
                    {["Id"] = correlation_id, ["name"] = tostring(name), ["Type"] = "AVALIABLE_RESULT"},
                    data
                )
    ...
    if #args > 0 then
        ret:ValidIf(
            function(self, data)
                ...
                xmlReporter.AddMessage(
                    "EXPECT_HMICALL",
                    {["Id"] = data.id, ["name"] = tostring(methodName), ["Type"] = "EXPECTED_RESULT"},
                    arguments
                )
                xmlReporter.AddMessage(
                    "EXPECT_HMICALL",
                    {["Id"] = data.id, ["name"] = tostring(methodName), ["Type"] = "AVALIABLE_RESULT"},
                    data.params
                )
...
-- @tparam table ... Expectation parameters
-- @treturn Expectation Expectation
function EXPECT_ANY_SESSION_NOTIFICATION(funcName, device, ...)
    local args = table.pack(...)
    local event = events.Event()
    event.matches = function(_, data)
        return data.rpcFunctionId == functionId[funcName]
    end
    local ret = Expectation(funcName .. " notification", device)
    if #args > 0 then
        ret:ValidIf(
            function(self, data)
                local arguments
                ...
                xmlReporter.AddMessage(
                    ...
                    {["name"] = tostring(funcName), ["Type"] = "EXPECTED_RESULT"},
                    arguments
                )
                xmlReporter.AddMessage(
                    "EXPECT_ANY_SESSION_NOTIFICATION",
                    {["name"] = tostring(funcName), ["Type"] = "AVALIABLE_RESULT"},
                    data.payload
                )
                if (not _res) then
                    return _res, _err
...
-- @tparam table ... Expectation parameters
-- @treturn Expectation Expectation
function EXPECT_ANY_SESSION_RESPONSE(correlationId, device, ...)
    ...
    local ret = Expectation("response to " .. correlationId, device)
    if #args > 0 then
        ret:ValidIf(
            function(self, data)
                local arguments
...
-- @treturn Expectation Expectation
function EXPECT_ANY(session)
    xmlReporter.AddMessage(debug.getinfo(1, "n").name, "")
    return session:ExpectAny()
end
