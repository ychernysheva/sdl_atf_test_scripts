...
function testCasesForArrayStringParameterInNotification:verify_Array_String_Parameter(Notification, Parameter, Boundary,  ElementBoundary, Mandatory, IsStringElementAcceptedSpecialCharacter)
		...
		if Boundary[1] > 0 then
			...
			commonFunctions:TestCaseForNotification(self, Notification, Parameter, "IsLowerBound", value, "SUCCESS")
		else
		...
		commonFunctions:TestCaseForNotification(self, Notification, Parameter, "IsUpperBound", value, "SUCCESS")
		
		--4. IsOutLowerBound/IsEmpty
		if Boundary[1] ==1 then
			local value = commonFunctions:createArrayString(Boundary[1]-1)
			commonFunctions:TestCaseForNotification(self, Notification, Parameter, "IsOutLowerBound_IsEmpty", value, "GENERIC_ERROR")
		
		elseif Boundary[1] >1 then
			local value = commonFunctions:createArrayString(Boundary[1]-1)
			commonFunctions:TestCaseForNotification(self, Notification, Parameter, "IsOutLowerBound", value, "GENERIC_ERROR")
			
			commonFunctions:TestCaseForNotification(self, Notification, Parameter, "IsEmpty", {}, "GENERIC_ERROR")			
		else
		...
		commonFunctions:TestCaseForNotification(self, Notification, Parameter, "IsOutUpperBound", value, "GENERIC_ERROR")
		
		--6. IsWrongType
		commonFunctions:TestCaseForNotification(self, Notification, Parameter, "IsWrongDataType", 123, "GENERIC_ERROR")
