...
---------------------------------------------------------------------------------------------
Test = require('user_modules/OnButtonPress_connecttest')
...
[[					"OnButtonEvent": {
	...
	--local PTName = testCasesForPolicyTable:createPolicyTableFile(PermissionLinesForBase4, PermissionLinesForGroup1, PermissionLinesForApplication, {"OnButtonEvent", "OnButtonPress", "Show", "SubscribeButton"})	
...
local function SpecialResponseChecks()
	...
	function Test:OnButtonPress_InvalidJsonSyntax()
		...
		self.hmiConnection:Send('{"jsonrpc";"2.0","params":{"mode":"SHORT","name":"PRESET_0"},"method":"Buttons.OnButtonPress"}')
	...
	function Test:OnButtonPress_FakeParams()
		...
		self.hmiConnection:SendNotification("Buttons.OnButtonPress",{name = "PRESET_0", mode = "SHORT", fake = 123})	
	...
	function Test:OnButtonPress_FakeParameterIsFromAnotherAPI()
		...
		self.hmiConnection:SendNotification("Buttons.OnButtonEvent",{name = "PRESET_0", mode = "BUTTONDOWN"})
		self.hmiConnection:SendNotification("Buttons.OnButtonEvent",{name = "PRESET_0", mode = "BUTTONUP"})
		self.hmiConnection:SendNotification("Buttons.OnButtonPress",{name = "PRESET_0", mode = "SHORT", sliderPosition = 1})	
	...
	function Test:OnButtonPress_mode_IsMissed()
		...
		self.hmiConnection:SendNotification("Buttons.OnButtonEvent",{name = "PRESET_0", mode = "BUTTONDOWN"})
		self.hmiConnection:SendNotification("Buttons.OnButtonEvent",{name = "PRESET_0", mode = "BUTTONUP"})
		self.hmiConnection:SendNotification("Buttons.OnButtonPress",{name = "PRESET_0"})	
	...
	function Test:OnButtonPress_SeveralNotifications_WithDifferentValues()
		...
		self.hmiConnection:SendNotification("Buttons.OnButtonEvent",{name = "PRESET_0", mode = "BUTTONUP"})
		self.hmiConnection:SendNotification("Buttons.OnButtonPress",{name = "PRESET_0", mode = "SHORT"})
		self.hmiConnection:SendNotification("Buttons.OnButtonPress",{name = "PRESET_0", mode = "LONG"})
		...
		--mobile side: expected OnButtonPress notification
		EXPECT_NOTIFICATION("OnButtonPress", 	{buttonName = "PRESET_0", buttonPressMode = "SHORT"},
												{buttonName = "PRESET_0", buttonPressMode = "LONG"})
		:Times(2)
	end
...
----------------------------------------------------------------------------------------------
	...
	local function ResultCodeChecks()
		...
		local function TC_OnButtonPress_DISALLOWED_USER_DISALLOWED(TestCaseName)
			Test[TestCaseName] = function(self)
	...
	----------------------------------------------------------------------------------------------
		...
		local PTFileName = testCasesForPolicyTable:createPolicyTableWithoutAPI("OnButtonPress")
		
		--Precondition: Update policy table
		testCasesForPolicyTable:updatePolicy(PTFileName)
		TC_OnButtonPress_DISALLOWED_USER_DISALLOWED("OnButtonPress_IsNotExistInPT_DISALLOWED")
...
[[					"OnButtonPress": {
		...
		--Send notification and check it is ignored		
		TC_OnButtonPress_DISALLOWED_USER_DISALLOWED("OnButtonPress_UserHasNotConsentedYet_DISALLOWED")
	...
	----------------------------------------------------------------------------------------------	
		...
		--Send notification and check it is ignored
		TC_OnButtonPress_DISALLOWED_USER_DISALLOWED("OnButtonPress_USER_DISALLOWED")
	...
	----------------------------------------------------------------------------------------------
		...
		function Test:OnButtonPress_USER_ALLOWED()
...
--Requirement id in JAMA:
	...
	--2. SDLAQ-CRS-3068: OnButtonPress to media app in FULL
	--3. SDLAQ-CRS-3069: OnButtonPress to media app in LIMITED
	...
	-- function Test:Deactivate_App3_To_Background()
	
	-- 	--hmi side: sending BasicCommunication.OnAppDeactivated notification
	-- 	self.hmiConnection:SendNotification("BasicCommunication.OnAppDeactivated", {appID = Apps[3].appID, reason = "GENERAL"})
	...
	local function OnButtonPress_OnlyComesToFullOrLimitedHmiLevelApplication(TestCaseName, ButtonName)
		Test[TestCaseName] = function(self)
	...
	for i=1,#MediaButtons do
		OnButtonPress_OnlyComesToFullOrLimitedHmiLevelApplication("OnlyFullApplicationReceivesOnButtonPress" .. MediaButtons[i], MediaButtons[i])
	end
	----------------------------------------------------------------------------------------------
	...
	if commonFunctions:isMediaApp() then		
		...
		commonSteps:ChangeHMIToLimited()
		for i=1,#MediaButtons do
			OnButtonPress_OnlyComesToFullOrLimitedHmiLevelApplication("OnlyLimitedApplicationReceivesOnButtonPress"..MediaButtons[i], MediaButtons[i])
		end
