...
local function SendPerformInteraction(self, name, keyEvent, level, layout, session)
	...
	local parametersToSend = createPerformInteraction()
	local cid = nil
	if session == 1 then
	...
	if session == 1 then
		...
		self.mobileSession1:ExpectNotification(cid, { success = true, resultCode = "SUCCESS", manualTextEntry="abc" })
	end
...
---------------------------------------------------------------------------------------------
	...
	--Description: Activation App by sending SDL.ActivateApp	
		function Test:ActivationApp()
			local HMIappID = self.applications["Test Application"]
...
function Test:PreconditionDeactivateApp2()
	testName("Deactivate App", "pre")
	self.hmiConnection:SendNotification("BasicCommunication.OnAppDeactivated", {appID = HMIappID2, reason = "AUDIO"})
	self.mobileSession1:ExpectNotification(
		"OnHMIStatus",
		{hmiLevel = "BACKGROUND", audioStreamingState = "NOT_AUDIBLE", systemContext = "MAIN"})
	:Times(1)
...
function Test:UnregisterApplications()
	...
	self.mobileSession1:ExpectNotification("BasicCommunication.OnAppUnregistered", 
