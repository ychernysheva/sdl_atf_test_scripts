--Note: 
	--Update config.AppFolder value in config.lua file to real application folder on test machine before running
	
	--List of existing problems:
		--APPLINK-14212: DeleteFile response: spaceAvailable parameter is wrong value
	...
	--Description: Check filenames parameter
		...
		--Description: Check filenames parameter value in case there are more than 1000 files on app folder
			...
			function Test:ListFiles_filenames_OutUpperBound_1001()
				...
				:ValidIf (function(_,data)
					if #data.payload.filenames >= 1001 then
						print("filenames returns more than 1000 files")
						return false
					end								
			    end)
...
----------------------------------------------------------------------------------------------
	...
	--Description: processing API in different HMILevel
		...
		--Description: Check ListFiles request when application is in NONE HMI level
		
			-- Precondition: Change app to NONE HMI level
			commonSteps:DeactivateAppToNoneHmiLevel()
			
			function Test:ListFiles_NONE_HMI_Level()
		...
		--Description: Check ListFiles request when application is in LIMITED HMI level
			...
			if commonFunctions:isMediaApp() then
				...
				function Test:ListFiles_LIMITED_HMI_Level()
		...
		--Description: Check ListFiles request when application is in BACKGOUND HMI level

			-- Precondition 1: Change app to BACKGOUND HMI level
			commonTestCases:ChangeAppToBackgroundHmiLevel()
			
			function Test:ListFiles_BACKGROUND_HMI_Level()
