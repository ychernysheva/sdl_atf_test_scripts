...
function Test:Precondition_ConnectDevice_NonEmptyDeviceList()
	self:connectMobile()
...
function Test:Precondition_UpdateDeviceList_InPreviosIGNCycleWasConnectedDevice()
	-- connect device
	self:connectMobile()
...
function Test:Precondition_ConnectMobile_AfterDeviceIsAllowed()
	self:connectMobile()
...
--======================================================================================--
--Precondition: Restart SDL
RestartSDL("AfterIGNOFF_CosentedDevice")

--======================================================================================--
function Test:Precondition_ConnectMobile_AfterIGNOFF_CosentedDevice()
	self:connectMobile()
...
--======================================================================================--
function Test:UpdateDeviceList_AfterIGNOFF_CosentedDevice()
...
--Precondition: Restart SDL
RestartSDL("NotCosentedDevice")

function Test:Precondition_ConnectMobile_NotCosentedDevice()
	self:connectMobile()

	DelayedExp(1500)
end

function Test:Precondition_CheckDeviceStatus_NotCosentedDevice()
...
function Test:UpdateDeviceList_NotCosentedDevice()
self.hmiConnection:SendNotification("BasicCommunication.OnStartDeviceDiscovery")
				...
				elseif
					exp.occurences == 2 then 
						...
						local function DBQuery()
							...
							if not IsConsentedPTValue then
						    	self:FailTestCase(" DB query to device_consent_group.is_consented is failed")
							elseif tonumber(IsConsentedPTValue) ~= 0 then
					    		self:FailTestCase("is_consent value in PT is unexpected = '" .. tostring(IsConsentedPTValue) ..  "', expected value is '1'")
					    	end
						end
...
--Precondition: Restart SDL, delete storage folder 
RestartSDL("DeleteStorageFolder_AfterClosingCinnectionWithRegisteredApp", true)

function Test:Precondition_ConnectMobile_AfterClosingCinnectionWithRegisteredApp()
	self:connectMobile()
...
function Test:UpdateDeviceList_AfterClosingCinnectionWithRegisteredApp()
	-- close connection
	self.mobileConnection:Close()
...
function Test:Precondition_CreateFirstConnection_DifferentDeviceId()
	self:connectMobile()
...
function Test:Precondition_CreateConnection_afterAllAppsAreUnregisteredFromDevice()
	self:connectMobile()
end

function Test:Precondition_RegisterApplication_afterAllAppsAreUnregisteredFromDevice()
	self.mobileSession = mobile_session.MobileSession(
	      self,
	      self.mobileConnection)
