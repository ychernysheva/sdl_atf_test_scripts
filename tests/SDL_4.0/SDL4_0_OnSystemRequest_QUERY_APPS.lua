Test = require('user_modules/connecttest_SDL4_0_query_apps')
...
local function HeaderValuesCheck(binaryData, Table)
	...
	local timeout_after_x_seconds = "sqlite3 " .. tostring(config.pathToSDL) .. "storage/policy.sqlite \"SELECT timeout_after_x_seconds FROM module_config WHERE rowid = 1\""
	...
	if timeout_after_x_secondsValue then
		...
		local ExpectParamsValues = { 
			...
			{paramName = "ReadTimeout", paramValue = timeout_after_x_secondsValue}, 
...
Precondition_UnregisterRegisterApp("OnSystemRequestQueryAppsAfterAppIsRegisteredForegroundOnMobile")
...
function Test:Precondition_OpenFirstConnectionCreateSession()
  ...
  function Test:Precondition_OpenSecondConnectionCreateSession()
    ...
    self.mobileSession1:ExpectEvent(events.connectedEvent, "Connection started")
  ...
  function Test:OnSystemRequestQueryAppsOnFirstDevice()
	...
	EXPECT_NOTIFICATION("OnSystemRequest",
	    ...
	    :Do(function(_,data)
	        ...
	        elseif
	        	data.payload.requestType == "LOCK_SCREEN_ICON_URL" then 
	        		SendingOnHMIStatusFromMobile(self)
	        end
	    end)
  ...
  function Test:OnSystemRequestQueryAppsOnSecondDevice()
  	userPrint(34, "=================================== Test  Case ===================================")
  	AppRegistration(self, self.mobileSession1 , config.application2.registerAppInterfaceParams.appName , config.application2.registerAppInterfaceParams.appID)
	...
	self.mobileSession1:ExpectNotification("OnSystemRequest",
	    ...
	    :Do(function(_,data)
	    	if 
 			...
 			elseif
	        	data.payload.requestType == "LOCK_SCREEN_ICON_URL" then 
	        		SendingOnHMIStatusFromMobile(self, _, _, self.mobileSession1)
	        end
...
local function TC_APPLINK_17903()
	...
	function Test:APPLINK_17903_Step2_OnSystemRequest_QueryApps_IsNotSent()
		userPrint(34, "=================================== Test  Case ===================================")
		--SendingOnHMIStatusFromMobile(self)
		SendingOnHMIStatusFromMobile(self, "BACKGROUND", "NOT_AUDIBLE", self.mobileSession )
