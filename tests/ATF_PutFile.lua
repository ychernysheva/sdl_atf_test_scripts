...
----------------------------------------------------------------------------------------------
	...
	--Description:TC's check all resultCodes values in pair with success value
		...
		--Description: Check OUT_OF_MEMORY result code
			...
			--Requirement id in JAMA: SDLAQ-CRS-707, APPLINK-10346
			...
			for i=1, 10 do			
				Test["Precondition_MakeFullStorage"] = function(self)
					--mobile side: sending PutFile request
		...
		--Description: Check APPLICATION_NOT_REGISTERED result code 
			...
			function Test:PutFile_ApplicationNotRegisterSuccessFalse() 
				local paramsSend = putFileAllParams()
								
				--mobile side: send PutFile request 	 	
				local CorIdPutFile = self.mobileSession2:SendRPC("PutFile", paramsSend)
								
				--mobile side: expect PutFile response
				self.mobileSession2:ExpectResponse(CorIdPutFile, { success = false, resultCode = "APPLICATION_NOT_REGISTERED" })
			end
					...
					if fileNamesValue ~= nil then
						for i=1, #fileNamesValue do
							if(fileNamesValue[i] == "checkListFile") then
								print(" \27[36m SDL provide file name in ListFiles response while file is just uploading. \27[0m ")
								return false
							end
						end
					else
				...
				function Test:PutFile_spaceAvailableZeroWhenSystemFileTrue() 
					...
					:ValidIf (function(_,data)
						if data.payload.spaceAvailable ~= 0 then
							print(" \27[36m PutFile response with spaceAvailable = ".. data.payload.spaceAvailable .." when systemFile=true \27[0m ")
							return false
						else
							return true
						end
					...
					:ValidIf (function(_,data)						
						...
						if fileNamesValue ~= nil then
							...
							if FileExisting == false then
								print (" \27[36m Persisted File is deleted after re-registering the app \27[0m ")
								return false
							elseif 
								isExist == true then
								return true
							else
								print (" \27[36m Persisted File is absent in ListFiles response \27[0m ")
								return false
							end
						elseif
							...
							print (" \27[36m Persisted File is present in storage folder, but ListFiles response came without list of files\27[0m ")
							return false
						else
							print (" \27[36m Persisted File is deleted after re-registering the app \27[0m ")
							return false
				...
				self.mobileSession7654321:ExpectResponse(cid, { success = true, resultCode = "SUCCESS" })
					:ValidIf(function ()
						...
						if not DirectoryExistResult then
							commonFunctions:userPrint(31, "Folder '7654321_" .. config.deviceMAC .."' do not exist")
						end
			...
			function Test:AppFolderIsAbsentAfterIGNOFF()
				...
				if DirectoryExistResult then
					...
					if RmFolder ~= true then
						commonFunctions:userPrint(31, "Folder '7654321_" .. config.deviceMAC .."' is not deleted")
					end
		...
		--Description: APPLINK-18360 - Checks that SDL create app folder with name which consist from <appID>_<deviceID> and does not delete this folder after ignition OFF-ON if persistent file was stored.
			...
			function Test:AppFolderIsNotDeletedAfterIGNOFF()
				...
				if not DirectoryExistResult then
					...
					local RmFolder  = assert( os.execute( "rm -rf " .. tostring(directoryPath)))
					if RmFolder ~= true then
						commonFunctions:userPrint(31, "Folder '54321_" .. config.deviceMAC .."' is not deleted")
					end
				end
			end
		
		--Begin Test case SequenceCheck.8
		--Description: Checks that SDL create different folder for the different app on the same transports
			...
			function Test:PutFile_CretedFirstAppStorageFolder()
				--mobile side: sending PutFile request
			...
			-- Add file from second app and check creaton app folder
			function Test:PutFile_CretedFirstAppStorageFolder()
				--mobile side: sending PutFile request
...
--[==[TODO: Update according to APPLINK-11264				
		...
		--Description: Checking Enable Image Upload After Specifying Usage
			...
			13. 	In case SDL starts moving data file into sharedMemory sub-directory AND sharedMemory sub-directory has already stored data file with the same name SDL must:
				...
				--Begin Test case SequenceCheck.6.6.1
				...
				--End Test case SequenceCheck.6.6.1
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.6.2
				...
				--End Test case SequenceCheck.6.6.2	
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.6.3
				...
				--End Test case SequenceCheck.6.6.3
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.6.4
				...
				--End Test case SequenceCheck.6.6.4
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.6.5
				...
				--End Test case SequenceCheck.6.6.5
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.6.6
				...
				--End Test case SequenceCheck.6.6.6
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.6.7
				...
				--End Test case SequenceCheck.6.6.7
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.6.8
				...
				--End Test case SequenceCheck.6.6.8
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.6.9
				...
				--End Test case SequenceCheck.6.6.9				
			...
			10. 	In case app uploads data file after using it in RPC (from list of affected RPCs) SDL must:
				...
				--Begin Test case SequenceCheck.6.9.1
				...
				--End Test case SequenceCheck.6.9.1
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.9.2
				...
				--End Test case SequenceCheck.6.9.2
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.9.3
				...
				--End Test case SequenceCheck.6.9.3
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.9.4
				...
				--End Test case SequenceCheck.6.9.4
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.9.5
				...
				--End Test case SequenceCheck.6.9.5
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.9.6
				...
				--End Test case SequenceCheck.6.9.6
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.9.7
				...
				--End Test case SequenceCheck.6.9.7
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.9.8
				...
				--End Test case SequenceCheck.6.9.8
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.9.9
				...
				--End Test case SequenceCheck.6.9.9				
			...
			11. In case app uploads data file after using it in RPC (from list of excepted RPCs) SDL must NOT move data file to sharedMemory sub-directory 
				...
				--Begin Test case SequenceCheck.6.10.1
				...
				--End Test case SequenceCheck.6.10.1
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.10.2
				...
				--End Test case SequenceCheck.6.10.2
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.10.3
				...
				--End Test case SequenceCheck.6.10.3				
			...
			--Begin Test case SequenceCheck.6.13
			--Description: 	
			--[[
			...
			--End Test case SequenceCheck.6.13
			
			-----------------------------------------------------------------------------------------
			
			--Begin Test case SequenceCheck.6.14
			--Description: 	
			--[[
			...
			--End Test case SequenceCheck.6.14
			
			-----------------------------------------------------------------------------------------
			
			--Begin Test case SequenceCheck.6.15
			--Description: 	
			--[[
			...
			--End Test case SequenceCheck.6.15
			
			-----------------------------------------------------------------------------------------
			
			--Begin Test case SequenceCheck.6.16
			--Description: 	
			--[[
			...
			--End Test case SequenceCheck.6.16
			-----------------------------------------------------------------------------------------
			
			--Begin Test case SequenceCheck.6.17
			--Description: 	
			...
			--End Test case SequenceCheck.6.17
						
			-----------------------------------------------------------------------------------------
			
			--Begin Test case SequenceCheck.6.18
			--Description: 	
			--[[
			...
			--]]
				--Begin Test case SequenceCheck.6.18.1
				...
				--End Test case SequenceCheck.6.18.1
				
				-----------------------------------------------------------------------------------------
				
				--Begin Test case SequenceCheck.6.18.2
				...
				--End Test case SequenceCheck.6.18.2				
			--End Test case SequenceCheck.6.18			
...
--]==]		
		...
		local function Task_APPLINK_15934()
			...
			--Put Files - fileType: "GRAPHIC_BMP","GRAPHIC_JPEG" ,"GRAPHIC_PNG"
				local  fileTypeValues = {"GRAPHIC_BMP","GRAPHIC_JPEG" ,"GRAPHIC_PNG"}
				local  fileValues = {"bmp_6kb.bmp", "jpeg_4kb.jpg", "icon.png"}
				local spaceAvailableValue = 0
				for i=1,#fileTypeValues do
					Test["PutFile_App5_fileType" .. tostring(fileTypeValues[i])] = function(self)
						...
						:ValidIf (function(_,data)
						local app5StoragePath = config.SDLStoragePath.."5".. "_" .. config.deviceMAC.. "/"
...
----------------------------------------------------------------------------------------------
	...
	--Description: processing API in different HMILevel
		...
		--Description: 
			...
			--Description: SDL process PutFile request on LIMITED HMI level(for media app)
			...
			if 
				...
				function Test:PutFile_HMILevelLimited()					
					self:putFileSuccess(putFileAllParams())
				end
			...
			--Description: SDL process PutFile request on BACKGROUND HMI level				
			...
			elseif
				...
				function Test:PutFile_HMILevelBackground()
					self:putFileSuccess(putFileAllParams())
				end
