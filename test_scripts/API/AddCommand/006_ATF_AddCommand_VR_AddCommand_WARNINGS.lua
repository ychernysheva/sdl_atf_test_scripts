...
-- in this particular test it is checked case when SDL sends WARNINGS (success:true) to mobile app in case HMI respond WARNINGS to VR.AddCommand and UI.AddCommand any successfull result code to UI.AddCommand
-- 1. Used preconditions: App is registered and activated SUCESSFULLY
-- 2. Performed steps:
...
local resultCodes = {"SUCCESS", "WARNINGS", "WRONG_LANGUAGE", "RETRY", "SAVED"}
  for i=1,#resultCodes do
    Test["TestStep_AddCommand_VR_AddCmd_WARNINGS_and_UI_AddCmd_"..resultCodes[i]] = function(self)
      ...
      :Do(function(_,data)
        self.hmiConnection:SendResponse(data.id, "UI.AddCommand", resultCodes[i], {})
      end)
