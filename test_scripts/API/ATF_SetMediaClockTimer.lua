...
policyTable:precondition_updatePolicy_AllowFunctionInHmiLeves({"BACKGROUND", "FULL", "LIMITED"},"SetMediaClockTimer")
...
for i=1,#updateMode do
  for j=1,#audioStreamingIndicatorStates do
    Test["SetMediaClockTimer_PositiveCase_" .. tostring(updateMode[i]) .. "_" .. tostring(audioStreamingIndicatorStates[j]) .. "_SUCCESS"] = function(self)
      countDown = 0
      ...
      local cid = self.mobileSession:SendRPC("SetMediaClockTimer",
        {
          ...
          audioStreamingIndicator = audioStreamingIndicatorStates[j]
        })
...
function Test:SetMediaClockTimer_missing_updateMode_with_audioStreamingIndicator_INVALID_DATA()
...
local function Task_APPLINK_15934()
  ...
  --Description: check request with missing audioStreamingIndicator


  --Verification criteria: The request with "audioStreamingIndicator" and without updateMode value is sent, the INVALID_DATA response code is returned.
  function Test:SetMediaClockTimer_AudioStreamingIndicatorMandatoryMissing()
    --mobile side: sending SetMediaClockTimer request with values of endTime only
    local cid = self.mobileSession:SendRPC("SetMediaClockTimer",
      {
        audioStreamingIndicator="PLAY_PAUSE"
  ...
  --End of TC CommonRequestCheck.15
  -----------------------------------------------------------------------------------------
end
Task_APPLINK_15934()
...
--Description: check startTime.seconds parameter value is in bound with Audio Streaming Indicator

for i=1,#InBound60 do
  for j =1, #updateMode do
    for k =1, #audioStreamingIndicatorStates do
      Test["SetMediaClockTimer_startTime_seconds_InBound_" .. tostring(InBound60[i]) .."_"..tostring(updateMode[j]) .."_"..tostring(audioStreamingIndicatorStates[k]).."_SUCCESS"] = function(self)
        countDown = 0
        if updateMode[j] == "COUNTDOWN" then
          countDown = -1
        end
...
--Description: check startTime.minutes parameter value is in bound with Audio Streaming Indicator

for i=1,#InBound60 do
  for j =1, #updateMode do
    for k =1, #audioStreamingIndicatorStates do
      Test["SetMediaClockTimer_startTime_minutes_InBound_" .. tostring(InBound60[i]) .."_"..tostring(updateMode[j]) .."_"..tostring(audioStreamingIndicatorStates[k]).."_SUCCESS"] = function(self)
        countDown = 0
        if updateMode[j] == "COUNTDOWN" then
          countDown = -1
        end
...
--Description: check startTime.hours parameter value is in bound with Audio Streaming Indicator

for i=1,#InBound60 do
  for j =1, #updateMode do
    for k =1, #audioStreamingIndicatorStates do
      Test["SetMediaClockTimer_startTime_hours_InBound_" .. tostring(InBound60[i]) .."_"..tostring(updateMode[j]) .."_"..tostring(audioStreamingIndicatorStates[k]).."_SUCCESS"] = function(self)
        countDown = 0
        if updateMode[j] == "COUNTDOWN" then
          countDown = -1
        end
...
--Description: check endTime.seconds parameter value is in bound with Audio Streaming Indicator

for i=1,#InBound60 do
  for j =1, #updateMode do
    for k =1, #audioStreamingIndicatorStates do
      Test["SetMediaClockTimer_endTime_seconds_InBound_" .. tostring(InBound60[i]) .."_"..tostring(updateMode[j]) .."_"..tostring(audioStreamingIndicatorStates[k]).."_SUCCESS"] = function(self)
        countDown = 0
        if updateMode[j] == "COUNTDOWN" then
          countDown = -1
        end
...
--Description: check endTime.minutes parameter value is in bound with Audio Streaming Indicator

for i=1,#InBound60 do
  for j =1, #updateMode do
    for k =1, #audioStreamingIndicatorStates do
      Test["SetMediaClockTimer_endTime_minutes_InBound_" .. tostring(InBound60[i]) .."_"..tostring(updateMode[j]) .."_"..tostring(audioStreamingIndicatorStates[k]).."_SUCCESS"] = function(self)
        countDown = 0
        if updateMode[j] == "COUNTDOWN" then
          countDown = -1
        end
...
--Description: check endTime.hours parameter value is in bound with Audio Streaming Indicator

for i=1,#InBound60 do
  for j =1, #updateMode do
    for k =1, #audioStreamingIndicatorStates do
      Test["SetMediaClockTimer_endTime_hours_InBound_" .. tostring(InBound60[i]) .."_"..tostring(updateMode[j]) .."_"..tostring(audioStreamingIndicatorStates[k]).."_SUCCESS"] = function(self)
        countDown = 0
        if updateMode[j] == "COUNTDOWN" then
          countDown = -1
        end
...
function Test:SetMediaClockTimer_updateMode_IsInvalidValue_WrongValue_Or_nonexistent_AudioStreamingIndicator_INVALID_DATA()
...
--Begin test case NegativeRequestCheck.2.1.5
--Description: 5.1.The request with empty "audioStreamingIndicator" is sent , the response with INVALID_DATA result code is returned.

function Test:SetMediaClockTimer_audioStreamingIndicator_IsInvalidValue_Empty_INVALID_DATA()
...
--End test case NegativeRequestCheck.2.1.5
-----------------------------------------------------------------------------------------
...
for i = 1, #resultCode do
  for j =1, #updateMode do
    Test["UI_SetMediaClockTimer_Response_resultCode_".. tostring(resultCode[i]) .."_"..tostring(updateMode[j]).."_"..tostring(resultCode[i]) .. "_" .. tostring(audioStreamingIndicatorStates[k]) ] = function(self)
      countDown = 0
      if updateMode[j] == "COUNTDOWN" then
        countDown = -1
      end
...
for j =1, #updateMode do
  for k =1, #audioStreamingIndicatorStates do
    Test["SetMediaClockTimer_RequestWithoutUIResponsesFromHMI_" ..tostring(updateMode[j]).."_" .. tostring(audioStreamingIndicatorStates[k]) .. "_GENERIC_ERROR"] = function(self)
      countDown = 0
      if updateMode[j] == "COUNTDOWN" then
        countDown = -1
      end
...
for j =1, #updateMode do
  for k =1, #audioStreamingIndicatorStates do
    Test["SetMediaClockTimer_UI_InvalidStructureOfResponse_" ..tostring(updateMode[j]).."_" .. tostring(audioStreamingIndicatorStates[k]).."_GENERIC_ERROR"] = function(self)
      countDown = 0
      if updateMode[j] == "COUNTDOWN" then
        countDown = -1
      end
...
for j =1, #updateMode do
  for k =1, #audioStreamingIndicatorStates do
    Test["SetMediaClockTimer_UI_SeveralResponseToOneRequest_" ..tostring(updateMode[j]).."_" .. tostring(audioStreamingIndicatorStates[k]) .. "_INVALID_DATA"] = function(self)
      countDown = 0
      if updateMode[j] == "COUNTDOWN" then
        countDown = -1
      end
...
for j =1, #updateMode do
  for k =1, #audioStreamingIndicatorStates do
    Test["SetMediaClockTimer_UI_ResponseWithFakeParamater" ..tostring(updateMode[j]).."_" .. tostring(audioStreamingIndicatorStates[k]) .. "_SUCCESS"] = function(self)
      countDown = 0
      if updateMode[j] == "COUNTDOWN" then
        countDown = -1
      end
...
for j =1, #updateMode do
  for k =1, #audioStreamingIndicatorStates do
    Test["SetMediaClockTimer_UI_ResponseWithParamatersOfOtherRequest" ..tostring(updateMode[j]).."_" .. tostring(audioStreamingIndicatorStates[k]) .. "_SUCCESS"] = function(self)
      countDown = 0
      if updateMode[j] == "COUNTDOWN" then
        countDown = -1
      end
...
for j =1, #updateMode do
  for k =1, #audioStreamingIndicatorStates do
    Test["SetMediaClockTimer_UI_ResponseWithWrongHMICorrelationId_" ..tostring(updateMode[j]).."_" .. tostring(audioStreamingIndicatorStates[k]) .. "_GENERIC_ERROR"] = function(self)
      countDown = 0
      if updateMode[j] == "COUNTDOWN" then
        countDown = -1
      end
...
for j =1, #updateMode do
  for k =1, #audioStreamingIndicatorStates do
    Test["SetMediaClockTimer_UI_WrongResponseWithCorrectHMICorrelationId_" ..tostring(updateMode[j]).."_" .. tostring(audioStreamingIndicatorStates[k]).. "_GENERIC_ERROR"] = function(self)
      countDown = 0
      if updateMode[j] == "COUNTDOWN" then
        countDown = -1
      end
