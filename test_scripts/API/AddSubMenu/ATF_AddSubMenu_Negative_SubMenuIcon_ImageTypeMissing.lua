...
--
-- Description:
-- 	Mobile app sends AddSubMenu with "subMenuIcon" without imageType
-- 1. Used preconditions:
-- 	Delete files and policy table from previous ignition cycle if any
-- 	Start SDL and HMI
--	Activate application
-- 2. Performed steps:
-- 	Send AddSubMenu RPC with "subMenuIcon" without imageType
--
-- Expected result:
...
function Test:Precondition_ActivateApp()
  ...
  if data.result.isSDLAllowed ~= true then
    ...
    :Do(function(_,data1)
    self.hmiConnection:SendResponse(data.id1,"BasicCommunication.ActivateApp", "SUCCESS", {})
    end)
    :Times(AtLeast(1))
    end)
  end
...
function Test:AddSubMenu_SubMenuIconImageTypeMissing()
