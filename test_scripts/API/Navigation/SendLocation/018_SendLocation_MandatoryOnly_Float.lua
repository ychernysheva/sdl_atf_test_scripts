...
--
-- Description:
-- App sends SendLocation request with only mandatory parameters with float values.
--
-- Steps:
-- mobile app requests SendLocation with mandatory parameters longitudeDegrees, latitudeDegrees with float values
--
-- Expected:
-- SDL must:
-- 1) transfer Navigation.SendLocation to HMI
...
local request_params = {
    longitudeDegrees = 1.1111111,
    latitudeDegrees = 1.1111111
}
...
--[[ Test ]]
runner.Title("Test")
runner.Step("SendLocation - mandatory only - float values", sendLocation, { request_params })
