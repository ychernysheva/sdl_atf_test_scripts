...
-- Update value of "steeringWeelLocation" parameter from "HMI_capabilities.json" to LEFT
-- In InitHMI_OnReady HMI replies with parameter steeringWeelLocation = CENTER to UI.GetCapabilities
--
-- 2. Performed steps
-- Register 4 new applications.
--
-- Expected result:
-- for each application: 
-- SDL->mobile: RegisterAppInterface_response steeringWeelLocation is provided equal to CENTER
---------------------------------------------------------------------------------------------
...
function Test:Precondition_FourthSession()
...
function Test:TestStep_RAI_FourthApp_steeringWheelLocation()
	local CorIdRegister = self.mobileSession3:SendRPC("RegisterAppInterface", config.application4.registerAppInterfaceParams)
		
	EXPECT_HMINOTIFICATION("BasicCommunication.OnAppRegistered", { application = { appName = config.application4.registerAppInterfaceParams.appName }})
	self.mobileSession3:ExpectResponse(CorIdRegister, { success=true, resultCode = "SUCCESS", hmiCapabilities = { steeringWheelLocation = "CENTER" } })
