...
-- in particular test it is checked case when TTS.SetGlobalProperties gets UNSUPPORTED_RESOURCE and UI.SetGlobalProperties gets WARNINGS
...
-- HMI -> SDL: UI.SetGlobalProperties (WARNINGS), TTS.SetGlobalProperties (UNSUPPORTED_RESOURCE)
--
-- Expected result:
...
function Test:TestStep_SetGlobalProperties_WARNINGS_to_UI_SGP_and_UNSUPPORTED_RESOURCE_to_TTS_SGP()
  ...
  :Do(function(_,data) self.hmiConnection:SendResponse(data.id, "TTS.SetGlobalProperties", "UNSUPPORTED_RESOURCE", {})
  end)
  ...
  :Do(function(_,data) self.hmiConnection:SendResponse(data.id, "UI.SetGlobalProperties", "WARNINGS", {})
  end)
