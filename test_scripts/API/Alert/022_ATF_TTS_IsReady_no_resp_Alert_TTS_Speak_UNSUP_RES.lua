...
-- in this particular test it is checked case when TTS.Speak gets UNSUPPORTED_RESOURCE and UI.Alert gets WARNINGS from HMI
--
...
-- 2. Performed steps:
-- MOB -> SDL: sends Alert
-- HMI -> SDL: TTS.Speak (UNSUPPORTED_RESOURCE), UI.Alert (WARNINGS)
--
-- Expected result:
...
function Test:TestStep_Alert_TTS_Speak_UNSUPPORTED_RESOURCE()
  ...
  :Do(function(_,data2)
    local function alertResponse()
      self.hmiConnection:SendResponse(data.id, "UI.Alert", "WARNINGS", {})
    end
  ...
  :Do(function(_,data3)
    ...
    local function ttsSpeakResponse()
      self.hmiConnection:SendResponse(data.id, "TTS.Speak", "UNSUPPORTED_RESOURCE", {info = "unsupported resource"})
    end
