...
-- in this particular test it is checked case when UI.Alert gets SUCCESS and TTS.Speak gets WARNINGS from HMI 
-- 1. Used preconditions: App is registered and activated SUCCESSFULLY
-- 2. Performed steps: 
-- MOB -> SDL: sends Alert
-- SDL -> HMI: resends UI.Alert and TTS.Speak to HMI
-- HMI -> SDL: UI.Alert (SUCCESS),  TTS.Speak (WARNINGS)
--
-- Expected result:
...
function Test:TestStep_Alert_TTS_Speak_WARNINGS()
  ...
  :Do(function(_,data2) self.hmiConnection:SendResponse(data2.id, "UI.Alert", "SUCCESS", { })
  end)
  ...
  :Do(function(_,data3) self.hmiConnection:SendResponse(data3.id, "TTS.Speak", "WARNINGS", { })
  end)
