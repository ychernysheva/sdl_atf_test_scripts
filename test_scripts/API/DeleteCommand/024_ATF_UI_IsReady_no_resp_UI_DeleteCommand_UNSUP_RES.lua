...
function Test:Precondition_AddCommand()
  ...
  :Do(function(_,data)
  self.hmiConnection:SendResponse(data.id, data.method, "SUCCESS", {}) end)
...
function Test:TestStep_DeleteCommand_UI_DelCmd_UNSUPPORTED_RESOURCE()
  ...
  :Do(function(_,data) self.hmiConnection:SendResponse(data.id, "UI.DeleteCommand", "UNSUPPORTED_RESOURCE", {info = "unsupported resource"}) end)
