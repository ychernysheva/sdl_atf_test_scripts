	...
	--Description:
		...
		--Description: Check processing UnregisterAppInterface request with fake parameter
			...
			function Test:RegisterAppInterface_Success()
				print("Fails of OnPermissionsChange notification is because of APPLINK-24902 - \"Genivi: Unexpected unregistering application at resumption after closing session.\"")
				RegisterApplication(self)
			end

			--ToDo: Shall be removed when APPLINK-24902
						    	...
						    	if
						        	data.result.isSDLAllowed ~= true then
						            	...
						            	--hmi side: expect SDL.GetUserFriendlyMessage response
					    			  	--UPDATED: Update after resolving APPLINK-16094 EXPECT_HMIRESPONSE(RequestId,{result = {code = 0, method = "SDL.GetUserFriendlyMessage"}})
					    			  	--EXPECT_HMIRESPONSE(RequestId)
						...
						:Do(function(_,data)
					    	...
					    	if
					        	data.result.isSDLAllowed ~= true then
					            	...
					            	--hmi side: expect SDL.GetUserFriendlyMessage response
				    			  	--UPDATED: Update after resolving APPLINK-16094 EXPECT_HMIRESPONSE(RequestIdEXPECT_HMIRESPONSE(RequestId,{result = {code = 0, method = "SDL.GetUserFriendlyMessage"}})
				    			  	--EXPECT_HMIRESPONSE(RequestId)
				    			  	EXPECT_HMIRESPONSE(RequestIdEXPECT_HMIRESPONSE(RequestId,{result = {code = 0, method = "SDL.GetUserFriendlyMessage"}}) )
						              	:Do(function(_,data)
