...
-----------------------------------------------------------------------------------------------
	--List of CRQs:	
		--CRQ #3) APPLINK-25042: [VR Interface] '..tested_method..'(false) -> HMI respond with successful resultCode to spitted RPC
		--CRQ #4) APPLINK-25043: [VR Interface] '..tested_method..'(false) -> HMI respond with errorCode to spitted RPC
	...
	local function case1_IsReady_availabe_false()	
		...
		local function StopStartSDL_HMI_MOBILE()
			...
			-- VehicleInfo: APPLINK-25305
			-- Navigation: APPLINK-25301
			Test["InitHMI_onReady_"..TestedInterface.."_IsReady_" ..tostring(TestCaseName)] = function(self)
		...
		-----------------------------------------------------------------------------------------------
		--CRQ #3) APPLINK-25042: [VR Interface] '..tested_method..'(false) -> HMI respond with successful resultCode to split RPC
		-- UI: APPLINK-25102
		-- TTS: APPLINK-25133
		-- VehicleInfo: Not applicable
		-- Navigation: Not applicable
		--Verification criteria:
			-- In case SDL receives '..tested_method..' (available=false) from HMI
			...
			local function Interface_IsReady_response_availabe_false_check_split_RPC_Other_Interfaces_Responds_SUCCESS(TestCaseName)
				...
				--for i = 1, 1 do
					for count_RPC = 1, #RPCs do
						...
						Test["TC6_"..TestCaseName .. "_"..mob_request.name.."_UNSUPPORTED_RESOURCE_true_Incase_OtherInterfaces_responds_" .. TestData[i].resultCode] = function(self)
								...
								for cnt_rpc = 1, #NotTestedInterfaces[cnt].usedRPC do
						 			...
						 			if (local_rpc == hmi_call.name) then
										...
										-- Update of verified params
						 				...
						 				--======================================================================================================
						 				print("TC6_ Waiting RPC: "..local_interface.."."..local_rpc)
		...
		-----------------------------------------------------------------------------------------------	
		--CRQ #4) APPLINK-25043: [VR Interface] (false) -> HMI respond with errorCode to split RPC
		-- UI:  APPLINK-25100
		-- TTS: APPLINK-25134
		-- VehicleInfo: NA
		-- Navigation: NA
			...
			local function Interface_IsReady_response_availabe_false_check_split_RPC_Other_Interfaces_Responds_Error(TestCaseName)
				...
				--for i = 1, 1 do
					for count_RPC = 1, #RPCs do
						...
						Test["TC7_"..TestCaseName .. "_"..mob_request.name.."_UNSUPPORTED_RESOURCE_true_Incase_UI_responds_" .. TestData[i].resultCode] = function(self)
