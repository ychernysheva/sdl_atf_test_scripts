...
------------------------------------ Common Variables ---------------------------------------
---------------------------------------------------------------------------------------------
local APIName = "PerformAudioPassThru" -- set request name
      ...
      elseif
        level == "LIMITED" then
          ...
          function Test:verify_SUCCESS_Case(RequestParams, level)
            ...
            local TTSSpeakParams = self:createTTSSpeakParameters(RequestParams)
          ...
          local function createInitialPrompt(n)
            local temp = {}
            for i = 1, n do
              temp[i] = {
          ...
          function Test:policyUpdate(policyFileName, consent, bAllowed)
            ...
            :Do(function(_,_)
              ...
              policyTable:precondition_updatePolicy_AllowFunctionInHmiLeves({"FULL", "LIMITED"}, APIName)
              ...
              function Test:PerformAudioPassThru_FakeParam()
                ...
                local UIParams = self:createUIParameters(params)
                local TTSSpeakParams = self:createTTSSpeakParameters(params)
...
--mobile side: sending PerformAudioPassThru request
local cid = self.mobileSession1:SendRPC(APIName, createRequest())
...
function Test:PerformAudioPassThru_OnResetTimeout()
  ...
  :Do(function(_,data)
      ...
      RUN_AFTER(OnResetTimeoutSending, 1000)
      RUN_AFTER(OnResetTimeoutSending, 2000)
      RUN_AFTER(OnResetTimeoutSending, 4000)
      RUN_AFTER(OnResetTimeoutSending, 6000)

      RUN_AFTER(ttsSpeakResponse, 8000)
    end)
