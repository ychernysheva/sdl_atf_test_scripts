...
-- in this particular test it is checked case when TTS.Speak gets Warnings and UI.PerformAudioPassThru gets UNSUPPORTED_RESOURCE from HMI
--
...
-- HMI -> SDL: TTS.Speak (WARNINGS), UI.PerformAudioPassThru (UNSUPPORTED_RESOURCE)
--
-- Expected result:
...
function Test:TestStep_PerformAudioPassThru_UI_PerformAudioPassThru_UNSUPPORTED_RESOURCE()
  local cor_id= self.mobileSession:SendRPC("PerformAudioPassThru",
    ...
    :Do(function(_,data3)
      local function UIPerformAoudioResponce()
        self.hmiConnection:SendResponse(data3.id, "UI.PerformAudioPassThru", "UNSUPPORTED_RESOURCE", {info = "unsupported resource"})
      end
