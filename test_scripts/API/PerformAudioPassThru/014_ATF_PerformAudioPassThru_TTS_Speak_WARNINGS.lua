...
-- Checks that SDL sends WARNINGS(success:true) to mobile app in case HMI respond WARNINGS(success:true) to TTS.Speak and ANY successfull result code to UI.PerformAudioPassThru
--
-- 1. Used preconditions:
-- App is registered and activated SUCCESSFULLY
-- 2. Performed steps:
-- MOB -> SDL: sends PerformAudioPassThru
-- HMI -> SDL: TTS.Speak (WARNINGS), UI.PerformAudioPassThru (cyclically checked cases fo result codes SUCCESS, WARNINGS, WRONG_LANGUAGE, RETRY, SAVED)
--
-- Expected result:
...
for i=1,#resultCodes do
  Test["TestStep_PerformAudioPassThru_TTS_Speak_WARNINGS_and_UI_PerformAudioPassThru_"..resultCodes[i]] = function(self)
    ...
    :Do(function(_,data3)
      local function UIPerformAoudioResponce()
        self.hmiConnection:SendResponse(data3.id, "UI.PerformAudioPassThru", resultCodes[i], {})
      end
