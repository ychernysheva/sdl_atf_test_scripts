...
-- in this test case when UI.PerformInteraction gets WARNINGS and VR.PerformInteraction gets ANY successfull result code is checked
--
-- 1. Used preconditions: App is registered and activated SUCCESSFULLY
-- 2. Performed steps:
-- MOB -> SDL: sends PerformInteraction
-- HMI -> SDL: UI.PerformInteraction (WARNINGS), VR.PerformInteraction (cyclically checked cases fo result codes SUCCESS, WARNINGS, WRONG_LANGUAGE, RETRY, SAVED)
--
-- Expected result:
...
for i=1,#resultCodes do
  Test["TestStep_PerformInteraction_UI_PerfInteract_WARNINGS_and_VR_PerfInteract_"..resultCodes[i]] = function(self)
    ...
    :Do(function(_,data3)
      ...
      self.hmiConnection:SendResponse(data3.id, "VR.PerformInteraction", resultCodes[i], {choiceID = 1001})
