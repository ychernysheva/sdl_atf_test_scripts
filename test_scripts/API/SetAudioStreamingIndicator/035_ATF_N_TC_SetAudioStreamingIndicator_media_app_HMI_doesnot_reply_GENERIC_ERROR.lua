...
-- SDL must respond with "GENERIC_ERROR, success:false" in case HMI does NOT respond during <DefaultTimeout>
--
...
-- SDL->mobile: SetAudioStreamingIndicator_response("GENERIC_ERROR", success:false)
---------------------------------------------------------------------------------------------
...
function Test:TestStep_SetAudioStreamingIndicator_GENERIC_ERROR_audioStreamingIndicator_PAUSE_HMI_doesnot_reply()
  ...
  :Do(function() -- Do not send HMI response
  end)

  EXPECT_RESPONSE(corr_id, { success = false, resultCode = "GENERIC_ERROR" })
end
