...
--
-- Description: Applying of the minsize and maxsize for array of VD, minvalue and maxvalue for array element
--   with type=Float

-- Precondition:
-- 1. Preloaded file contains VehicleDataItems for all RPC spec VD
-- 2. App is registered and activated
-- 3. PTU is performed, the update contains the custom VD items in VehicleDataItems with type=Float, array=true,
--   minsize and maxsize,minvalue and maxvalue for Root-Level
  ...
  if VDitem.name == "custom_vd_item11_struct" then
    for subVDkey, subVDitem in pairs(VDitem.params) do
      if subVDitem.name == "struct_element_5_array" then
        common.customDataTypeSample[VDkey].params[subVDkey].type = "Float"
      end
    end
  end
end
...
local paramsForChecking = { "custom_vd_item8_array_float", "custom_vd_item11_struct" }
local arrayMinSize = { }

-- parameter values for root element
local rootArrayMaxSizeMinValue = { }
local rootArrayMaxSizeMaxValue = { }
for i=1,common.VehicleDataItemsWithData.custom_vd_item8_array_float.maxsize do
  rootArrayMaxSizeMinValue[i] = common.VehicleDataItemsWithData.custom_vd_item8_array_float.minvalue
  rootArrayMaxSizeMaxValue[i] = common.VehicleDataItemsWithData.custom_vd_item8_array_float.maxvalue
end

local rootArrayOutOfMaxSize = common.cloneTable(rootArrayMaxSizeMaxValue)
table.insert(rootArrayOutOfMaxSize, common.VehicleDataItemsWithData.custom_vd_item8_array_float.maxvalue)

local rootArrayOutOfMaxValue = { common.VehicleDataItemsWithData.custom_vd_item8_array_float.maxvalue + 0.1 }
...
local childArrayOutOfMaxValue = { common.VehicleDataItemsWithData.custom_vd_item11_struct.params.struct_element_5_array.maxvalue + 0.1 }

--[[ Local Functions ]]
local function setNewArrayParams(pValueRootLevel, pValueChildLevel)
  common.VehicleDataItemsWithData.custom_vd_item8_array_float.value = pValueRootLevel
...
runner.Step("Update parameter values to minvalue, maxsize", setNewArrayParams,
...
runner.Step("Update parameter values to maxvalue, maxsize", setNewArrayParams,
...
runner.Step("Update parameter values to out of maxvalue", setNewArrayParams,
  { rootArrayOutOfMaxValue, childArrayOutOfMaxValue })
for _, vehicleDataName in pairs(paramsForChecking) do
  runner.Step("OnVehicleData maxvalue" .. vehicleDataName, common.onVD, { appSessionId, vehicleDataName, onVDNOTexpected })
end
