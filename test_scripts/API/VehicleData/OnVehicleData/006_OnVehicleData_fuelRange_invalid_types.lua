...
-- 2) Notification OnVehicleData with invalid type of parameters
-- SDL must:
...
local notParams = {
  type = {{ range = rangeDefault, type = 111 }},
  range = {{ type = typeDefault, range = "string" }},
  element = { "elemnt" },
  fuelRange = "string"
}
...
for k,value in pairs(notParams) do
  runner.Step("OnVehicleData_fuelRange_invalidType_" .. k, OnVehicleDataNotification,{ value })
end
