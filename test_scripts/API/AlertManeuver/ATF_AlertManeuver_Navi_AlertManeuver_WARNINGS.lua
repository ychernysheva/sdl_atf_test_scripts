...
-- in this particular test it is checked case when HMI sende WARNINGS to Navi.AlertManeuver
-- 1. Used preconditions: App is registered and activated SUCCESSFULLY
-- 2. Performed steps: 
-- MOB -> SDL: AlertManeuver
-- SDL -> HMI: resends AlertManeuver
-- HMI -> SDL: Navi.AlertManeuver (WARNINGS), TTS.Speak (SUCCESS)
--
-- Expected result:
...
function Test:TestStep_AlertManeuver()
  ...
  :Do(function(_,data) self.hmiConnection:SendResponse(data.id, "Navigation.AlertManeuver", "WARNINGS", { })
  end)
  ...
  :Do(function(_,data) 
    ...
    self.hmiConnection:SendError(data.id, "TTS.Speak", "SUCCESS")
