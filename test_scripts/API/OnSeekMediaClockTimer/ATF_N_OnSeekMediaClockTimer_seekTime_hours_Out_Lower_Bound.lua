...
--
-- 2. Performed steps:
-- a) HMI sends OnSeekMediaClockTimer notification to SDL with hours out lower bound value ("-1") of seekTime parameter.
--
-- Expected result:
...
function Test:TestStep_OnSeekMediaClockTimer_hours_Value_Of_seekTime_Is_Out_Lower_Bound()
 self.hmiConnection:SendNotification("UI.OnSeekMediaClockTimer",{seekTime =  {hours = -1, minutes = 1, seconds = 33}, appID = self.applications["Test Application"]})
 EXPECT_NOTIFICATION("OnSeekMediaClockTimer", {seekTime = {hours = -1, minutes = 1, seconds = 33}}):Times(0)
end
