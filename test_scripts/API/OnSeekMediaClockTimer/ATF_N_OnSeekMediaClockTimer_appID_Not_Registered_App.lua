...
-- [HMI_API] <param name="appID" type="Integer" mandatory="true">
...
--
-- 2. Performed steps:
-- a) HMI sends OnSeekMediaClockTimer notification to SDL with appID of not registered app.
--
-- Expected result:
-- a) SDL does not resend OnSeekMediaClockTimer notification to current registered mobile app.
---------------------------------------------------------------------------------------------
...
function Test:TestStep_OnSeekMediaClockTimer_With_appID_Of_Not_Registered_App()
 self.hmiConnection:SendNotification("UI.OnSeekMediaClockTimer",{seekTime = {hours = 30, minutes = 30, seconds = 30}, appID = self.applications["Test Application"]+1})
