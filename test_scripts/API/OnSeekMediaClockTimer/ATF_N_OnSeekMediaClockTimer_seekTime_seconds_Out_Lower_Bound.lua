...
--
-- 2. Performed steps:
-- a) HMI sends OnSeekMediaClockTimer notification to SDL with seconds out lower bound value ("-1") of seekTime parameter.
--
-- Expected result:
...
function Test:TestStep_OnSeekMediaClockTimer_seconds_Value_Of_seekTime_Is_Out_Lower_Bound()
 self.hmiConnection:SendNotification("UI.OnSeekMediaClockTimer",{seekTime =  {hours = 3, minutes = 3, seconds = -1}, appID = self.applications["Test Application"]})
 EXPECT_NOTIFICATION("OnSeekMediaClockTimer", {seekTime = {hours = 3, minutes = 3, seconds = -1}}):Times(0)
end
