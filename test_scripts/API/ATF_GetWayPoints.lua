...
--------------------------------------------------------------------------------------------------------
Test = require('user_modules/connecttest_GetWayPoints')
...
-- 1. Update Policy
	function Test:PolicyUpdate()
		...
		-- TODO: Update after APPLINK-22712 will be in used ATF
		 EXPECT_NOTIFICATION("OnSystemRequest")
		 	:Do(function(_,data)
				policyTable:updatePolicyGenivi(self, "files/PTU_ForGetWayPoints.json")
			 end)
...
--Description: Check negative request
	...
	--Description: All parameters missing
		...
		function Test:GetWayPoints_MissedAllParameters()
			--mobile side: sending GetWayPoints request		
			local cid = self.mobileSession:SendRPC("GetWayPoints", {} )			
...
-- [[TODO debug after resolving APPLINK-13101
	...
	--Description: Check resultCode WARNINGS
		...
		function Test:GetWayPoints_resultCode_WARNINGS()
			--mobile side: sending the request
			local RequestParams = Test:createRequest()
			local cid = self.mobileSession:SendRPC("GetWayPoints", RequestParams)
...
local Struct = Response.wayPoints[1].searchAddress
 Test["GetWayPoints_searchAddress_element"] = function(self)
...
local testData = {"addressLines",
			 ...
			 for i =1, #testData do
 stringParameterInResponse:verify_String_Parameter(Response, {"wayPoints", 1, "searchAddress", testData[i]}, {1, 500}, false)
end

 end


-- Check for structure of searchAddress parameter
-----------------------------------------------------------------------------------------------------------------------------------
  arrayStructParameterInResponse:verify_Array_Struct_Parameter(Response, {"searchAddress"}, {1,10}, Struct, false)
...
--Check GetWayPointse Response For Each App APPLINK-21890
----------------------------------------------------------------------------------------------
			...
			:Do(function(_,data)
				...
				if exp.occurences == 1 then
					...
					local function sendSecondRequest()				
						...
						EXPECT_RESPONSE(cid2, { success = false, resultCode = "SUCCESS"})
					end
