...
if config.application1.registerAppInterfaceParams.isMediaApplication then
	...
	ButtonNames_NoCUSTOM_BUTTON = {"OK","SEEKLEFT","SEEKRIGHT", "TUNEUP", "TUNEDOWN","PRESET_0", "PRESET_1", "PRESET_2", "PRESET_3", "PRESET_4", "PRESET_5", "PRESET_6", "PRESET_7", "PRESET_8", "PRESET_9"}
  	...
  	EXPECT_HMIEVENT(events.connectedEvent, "Connected websocket")
      	...
      	registerComponent("Buttons", 
				      		{"Buttons.OnButtonSubscription"--,
				      		 --"Buttons.IsReady" --Clarification logged under APPLINK-24358 if this should be developed
				      		}
				      	)
...
local function HMI_Send_Button_GetCapabilities_Response(Input_capabilities)
	...
	Test["HMISendsButtonGetCapabilitiesResponse"] = function(self)
		...
		--ExpectRequest("Buttons.IsReady",true,{}) -- See comment above 
...
--Requirement id in JIRA: 	
	...
	--Verification criteria
		...
		--2. SDL sends to HMI notification Buttons.IsReady
		--3. HMI sends to SDL: Buttons.IsReady (SUCCESS)
		--4. SDL sends to HMI: Buttons.GetCapabilities request 
		--5. HMI checks the Buttons capabilities and sends to SDL response Buttons.GetCapabilities ("capabilities", "presetBankCapabilities")
		--6. The validity of parametres is checked in RegisterAppInterface_response ("buttonCapabilities", "presetBankCapabilities")
...
--Begin Test suit buttonCapabilities
	...
	--Description: HMI sends Buttons.GetCapabilities response with all button names
		...
		commonFunctions:newTestCasesGroup("Test case: HMI sends Buttons.GetCapabilities with all button names NoCustomButton")	
		...
		for i =1, #ButtonNames_NoCUSTOM_BUTTON_OK do 
				--Print new line to separate new test cases group	
	...
	--Description: HMI sends Buttons.GetCapabilities response with some button names
		...
		for i =1, #ButtonNames_NoCUSTOM_BUTTON do 
		--Print new line to separate new test cases group
		commonFunctions:newTestCasesGroup("Test case: HMI sends Buttons.GetCapabilities with some button names NoCustomButton")	
	...
	--Description: HMI sends Buttons.GetCapabilities response with one button
		...
		for i= 1, #ButtonNames_NoCUSTOM_BUTTON_OK do
			--Print new line to separate new test case
			commonFunctions:newTestCasesGroup("Test case: HMI sends Buttons.GetCapabilities with one button NoCustomButton_OK - \"".. ButtonNames_NoCUSTOM_BUTTON_OK[i] .. "\"")	
...
-- Clarification logged under APPLINK-24358 if some specific negative scenarios are applicable 
