...
function Test:RemovePermissionsFromPT()
	...
	local RemovePermissionsFromPT = assert(os.execute("chmod 000  " .. tostring(config.pathToSDL) .. "policy.sqlite"))
...
function Test:StartSDL_GivePermissionsToPTAfterSDLStart()
	...
	local GivePermissionsToPT = assert(os.execute("chmod 600  " .. tostring(config.pathToSDL) .. "policy.sqlite"))
	os.execute("date")
...
function Test:ResporeIniFileToOriginal_DeleteStorageFolder()
	...
	local BackPermissionsForPT = assert(os.execute("chmod 755  " .. tostring(config.pathToSDL) .. "policy.sqlite"))

	if not BackPermissionsForPT then
		self:FailTestCase(" Command of back permissions for policy.sqlite is not success")
	end
