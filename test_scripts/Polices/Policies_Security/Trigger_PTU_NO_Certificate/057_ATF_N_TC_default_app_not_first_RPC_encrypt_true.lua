...
-- Send StartService(serviceType = 7 (RPC)) for first time
...
--
-- 2. Performed steps
-- Send second StartService(serviceType = 7 (RPC))
--
-- Expected result:
-- SDL should not trigger PTU
---------------------------------------------------------------------------------------------
...
function Test:Precondition_PTU_certificate_exist_RPC_ACK_encryption_true()
  local startserviceEvent = Event()
  startserviceEvent.matches =
  function(_, data)
