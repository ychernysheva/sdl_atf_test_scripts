...
local function getFunctionGroupsName()  
  local sql_select = "sqlite3 " .. tostring(SDLStoragePath) .. "policy.sqlite \"SELECT functional_group.name FROM app_group JOIN functional_group ON app_group.functional_group_id = functional_group.id WHERE app_group.application_id = '0000001'\""
...
function checNamesOfGroups()
  func_group_name = getFunctionGroupsName(self)
  commonFunctions:userPrint(34, "func_group_name")
    if func_group_name ~= "Base-4" and "Location-1" and "DrivingCharacteristics-3" and "VehicleInfo-3" then
