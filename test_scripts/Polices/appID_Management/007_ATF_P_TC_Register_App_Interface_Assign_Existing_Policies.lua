...
--[[ Test ]]
  ...
  function Test:CheckPermissions()
    local corId = self.mobileSession2:SendRPC("SetGlobalProperties")
    self.mobileSession2:ExpectResponse(corId, {sucess = true, resultCode = "SUCCESS" })
  end
  ...
  function Test:CheckPermissions()
    local corId = self.mobileSession2:SendRPC("SetGlobalProperties")
    self.mobileSession2:ExpectResponse(corId, {sucess = false, resultCode = "DISALLOWED" })
  end
