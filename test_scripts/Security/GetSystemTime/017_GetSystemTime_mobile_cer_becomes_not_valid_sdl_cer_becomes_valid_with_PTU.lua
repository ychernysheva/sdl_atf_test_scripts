...
-- 2) Mobile certificate is up to date and sdl certificate is not up to date
-- 3) SDL requests GetSystemTime
-- 4) Mobile certificate becomes not valid and sdl certificate becomes valid according to date/time from GetSystemTime response
-- SDL must:
-- 1) trigger PTU
...
runner.Step("Handshake with BC.GetSystemTime request from SDL", common.startServiceSecuredwithPTU,
	{ pData, serviceId, 1, systemTime, ptUpdateWithActualCer, 1 })
