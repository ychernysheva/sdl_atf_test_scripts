...
--        SDL should delete oldest icon from AppIconsFolder
--        SDL should save the new icon having AppIconsFolder not more than 1048576 bytes
---------------------------------------------------------------------------------------------
...
local oldFileToCheck = config.pathToSDL .. tostring("Icons/icon1.png")
...
local function makeAppIconsFolderFull(AppIconsFolder)
  ...
  sizeToFull = 524288 - sizeAppIconsFolderInBytes
  local i =1
...
local function checkFunction()
    ...
    if oldFileExistResult ~= false then
      commonFunctions:userPrint(31,"Oldest icon1.png is not deleted from AppIconsFolder. More space is occupied than default folder size(1Mb). ")
      status = false
    end
    return status
end
...
function Test.Precondition_configureAppIconsFolderMaxSize()
  checkSDLPathValue()
  local appIconsFolderMaxSizeValueToReplace = "524288"
